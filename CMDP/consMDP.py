...
from CMDP.dot import consMDP2dot, dot_to_svg
...
class ConsMDP:
    ...
    The variant used by default in CMDP can be controlled by def_EL_class.
    ...
    Important
    =========
    Functions that change the structure of the CMDP should always call
    ...
    def __init__(self):
        """Constructor method - CMDP class
        """
        self.name = None

        self.succ = []
        self.actions = [0] #here initialize 'actions=[0]' ensure that the lenth of actions starts from 1
...
class ActionData:
    """Object that holds the data of an action in the CMDP for a given
...
class Succ_iteraser(Succ_iter):
    ...
    Expects an CMDP `mdp` and state index `s`. The function erase
